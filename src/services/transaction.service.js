...
const { bannedNinRepo } = require("../repositories/device.repository");
...
async function createTransaction(ownerId, data) {
  ...
  if (collector.status !== "active") throw new Error("Collector account is not active");
  ...
  // Notify collector
  notify.transactionUpdate(data.collectorId, "assigned", data.deviceModel).catch(() => {});
  emailService.transactionAssignedEmail(owner, collector, tx).catch(() => {});
...
async function updateTransactionStatus(id, status, requestingUid) {
  ...
  if (tx.ownerId !== requestingUid) throw new Error("Only the device owner can update this transaction");
  if (tx.status !== "assigned") throw new Error("Transaction is already resolved");

  const updates = { status, resolvedAt: Date.now() };
  await transactionRepo.update(id, updates);

  // Handle theft — record on device registry and notify
  if (status === "theft") {
    await adjustTrustScore(tx.collectorId, "theft_report_on_user");
    notify.transactionUpdate(tx.collectorId, "theft", tx.deviceModel).catch(() => {});
  } else if (status === "returned" || status === "paid") {
    await adjustTrustScore(tx.collectorId, "transaction_completed");
  }

  notify.transactionUpdate(tx.collectorId, status, tx.deviceModel).catch(() => {});
